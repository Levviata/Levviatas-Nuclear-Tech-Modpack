	...
	public void onMissileImpact(RayTraceResult mop) { //TODO: demolish this steaming pile of shit
		...
		switch(type) {
            ...
            case VOLCANO:
                ...
                for(int x = -1; x <= 1; x++) for(int y = -1; y <= 1; y++) for(int z = -1; z <= 1; z++) world.setBlockState(posVolcano.setPos((int) Math.floor(posX + x), (int) Math.floor(posY + y), (int) Math.floor(posZ + z)), ModBlocks.volcanic_lava_block.getDefaultState());
